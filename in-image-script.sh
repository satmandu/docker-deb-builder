...
. /tmp/env.txt
    ...
    ARM64=on checkinstall -D --install=no --pkgname=rpiuserland --pkgversion="$(date +%Y%m):$(date +%Y%m%d%H%M)-git" --fstrans=yes -y 
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted
    dpkg-deb -R rpiuserland_*.deb extracted/
...
EOF
    ...
    mv extracted.deb rpiuserland_$(date +%Y%m%d%H%M)-git.deb
[[ $PKGUSERLAND ]] && cp rpiuserland_$(date +%Y%m%d%H%M)-git.deb /output/
    cp rpiuserland_$(date +%Y%m%d%H%M)-git.deb /mnt/var/cache/apt/archives/
