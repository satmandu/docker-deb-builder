...
apt-get -o dir::cache::archives="${apt_cache}" install checkinstall patchelf -y &>> /tmp/main.install.log 
...
# Chroot Apt Wrapper (for use from outside chroot)
...
# Chroot Dpkg Wrapper (for use from outside chroot)
...
waitfor "image_mount"
    # Apt concurrency manager wrapper via
    # https://askubuntu.com/posts/375031/revisions
    # Chroot Apt Wrapper (for use inside chroot)
...
# Chroot Dpkg Wrapper (for use inside chroot)
...
. /tmp/env.txt
    ...
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/ld.so.conf.d/
    echo '/opt/vc/lib' > extracted/etc/ld.so.conf.d/vc.conf 
    
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/environment.d
    cat  <<-EOF > "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/environment.d/10-vcgencmd.conf
	...
	LDPATH="/opt/vc/lib"
EOF
    chmod +x "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/environment.d/10-vcgencmd.conf
    
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/profile.d/
    cat <<-'EOF' > "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/profile.d/98-rpi.sh 
	# /etc/profile.d/98-rpi.sh
	# Adds Raspberry Pi Foundation userland binaries to path
	export PATH="$PATH:/opt/vc/bin:/opt/vc/sbin"
EOF
    chmod +x "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/profile.d/98-rpi.sh
       
    cat  <<-EOF > "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/ld.so.conf.d/00-vmcs.conf
	/opt/vc/lib
EOF
...
EOF
    chmod 0440 "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/sudoers.d/rpi
    # Add display forwarding to sudo as per https://askubuntu.com/a/414810/844422
    echo "* Adding X Display forwarding to sudo."
    cat <<-EOF >> "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/sudoers.d/display
	Defaults env_keep+="XAUTHORIZATION XAUTHORITY TZ PS2 PS1 PATH LS_COLORS KRB5CCNAME HOSTNAME HOME DISPLAY COLORS"
EOF
    chmod 0440 "${workdir}"/rpi-userland/build/arm-linux/release/extracted/etc/sudoers.d/display
    
    # Get non-arm64 vcdbg and patch for use on armhf
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted/usr/arm-linux-gnueabihf/lib
    cd "${workdir}"/rpi-userland/build/arm-linux/release/extracted/usr/arm-linux-gnueabihf/lib
    ...
    mkdir -p "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/
    cd "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/
    curl -OL https://github.com/Hexxeh/rpi-firmware/raw/master/vc/hardfp/opt/vc/bin/vcdbg
    chmod +x "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/vcdbg
    cp "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/vcdbg "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/vcdbg.orig
    patchelf --force-rpath --set-rpath "/usr/arm-linux-gnueabihf/lib" "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/vcdbg
    patchelf --set-interpreter /usr/arm-linux-gnueabihf/lib/ld-linux-armhf.so.3 "${workdir}"/rpi-userland/build/arm-linux/release/extracted/opt/vc/bin/vcdbg
    
    cd "${workdir}"/rpi-userland/build/arm-linux/release/
    ARM64=on checkinstall -D --install=no --exclude="${workdir}" --pkgname=rpiuserland --pkgversion="$(date +%Y%m):$(date +%Y%m%d)-${USERLANDREV}" --requires="libgcc1-armhf-cross,libstdc++6-armhf-cross,libc6-armhf-cross" --fstrans=yes -y &>> /tmp/"${FUNCNAME[0]}".compile.log
    ...
    mv "${workdir}"/rpi-userland/build/arm-linux/release/extracted.deb "${workdir}"/rpi-userland/build/arm-linux/release/rpiuserland_$(date +%Y%m%d)-${USERLANDREV}_arm64.deb &>> /tmp/"${FUNCNAME[0]}".compile.log
[[ $PKGUSERLAND ]] && cp "${workdir}"/rpi-userland/build/arm-linux/release/rpiuserland_$(date +%Y%m%d)-${USERLANDREV}_arm64.deb /output/ &>> /tmp/"${FUNCNAME[0]}".compile.log
    cp "${workdir}"/rpi-userland/build/arm-linux/release/rpiuserland_$(date +%Y%m%d)-${USERLANDREV}_arm64.deb /mnt/var/cache/apt/archives/ &>> /tmp/"${FUNCNAME[0]}".compile.log
